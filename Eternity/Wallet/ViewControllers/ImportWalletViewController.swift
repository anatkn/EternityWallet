        ...
        form
            ...
            +++ Section {
                ...
                header.onSetupView = { (view, section) -> Void in
                    view.label.textColor = Colors.purple
            ...
            <<< SegmentedRow<String>(Values.segment) {
                ...
                cell.segmentedControl.tintColor = Colors.purple
                for subview in (cell.segmentedControl.subviews ) {
                    subview.tintColor = Colors.purple
                    for subview1 in (subview.subviews) {
                        subview1.tintColor = Colors.purple
                    }
                }
            }
            ...
            // Keystore JSON
            +++ Section() { section in
                section.hidden = Eureka.Condition.function([Values.segment], { [weak self] _ in
            ...
            <<< AppFormAppearance.textArea(tag: Values.keystore) { [weak self] in
                $0.placeholder = (self?.viewModel.keystorePlaceholder)! + " (" + ImportSelectionType.keystore.footerTitle + ")"
            ...
            <<< AppFormAppearance.textArea(tag: Values.privateKey) { [weak self] in
                $0.placeholder = (self?.viewModel.privateKeyPlaceholder)! + " (" + ImportSelectionType.privateKey.footerTitle + ")"
                $0.textAreaHeight = .fixed(cellHeight: 140)
                $0.add(rule: RuleRequired())
            ...
            <<< ButtonRow(NSLocalizedString("importWallet.import.button.title", value: "Import", comment: "")) {
                ...
                cell.textLabel?.textColor = Colors.darkPurple
            }
